...
<div v-if="!loading">
  ...
  <div class="overflow-x-auto">
    <table class="min-w-full bg-blue-50 border border-gray-200 rounded-lg shadow-lg">
      ...
      <tbody>
        <tr v-for="(table, index) in tables" :key="index" class="hover:bg-blue-100 border-b">
          <td class="py-4 px-4 text-sm font-medium text-gray-900">{{ table.id }}</td>
          <td class="py-4 px-4 text-sm text-gray-500">{{ table.tableNumber }}</td>
          <td class="py-4 px-4 text-sm">
            <button @click="deleteTable(table.id)" class="bg-red-500 hover:bg-red-800 text-white font-bold py-1 px-3 rounded">
              Törlés
            </button>
          </td>
        </tr>
