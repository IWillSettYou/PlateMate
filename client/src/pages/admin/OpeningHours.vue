...
export default {
  ...
  data(){
    return {
      loading: true,
      currentComponent: "OpeningHourList"
    }
  },
  async mounted(){
    try {
      ...
      <nav class="bg-white dark:bg-gray-800 shadow dark:border-gray-700 p-4">
        <div class="flex items-center">
          ...
          <div class="flex space-x-6 mx-auto">
            <button @click="currentComponent = 'OpeningHourList'" class="ml-auto text-white bg-slate-600 hover:bg-slate-700 font-medium rounded-lg text-m px-4 py-2">
              OpeningHour List
            </button>
            <button @click="currentComponent = 'OpeningHourAdd'" class="ml-auto text-white bg-slate-600 hover:bg-slate-700 font-medium rounded-lg text-m px-4 py-2">
              OpeningHour Add
            </button>
          </div>
