...
export default {
  ...
  async mounted(){
    try {
        const response = await this.getFinishedOrders();
        try{
            ...
            <tbody>
              <tr v-for="(order, index) in orders" :key="index" class="hover:bg-blue-100 border-b">
                ...
                <td class="py-4 px-4 text-sm">
                  <button @click="rollbackUpdatedOrder(order.id)" class="bg-red-500 hover:bg-red-800 text-white font-bold py-1 px-3 rounded">
                    Vissza
                  </button>
                </td>
                <td class="py-4 px-4 text-sm">
                  <button @click="setServedOrder(order.id)" class="bg-green-500 hover:bg-green-800 text-white font-bold py-1 px-3 rounded">
                    Felszolgálva
                  </button>
                </td>
              </tr>
