...
const getAllSettings = async (req, res) => {
    try {
        ...
        if (!users.length) {
            return res.status(404).json({ message: "Nincsenek elérhető jogok." });
        }
...
const getSettingById = async (req, res) => {
    ...
    try {
        ...
        if (!user.length) {
            return res.status(404).json({ message: "A megadott jog nem található." });
        }
...
const deleteSetting = async (req, res) => {
    ...
    try {
        ...
        if (response.affectedRows === 0) {
            return res.status(404).json({ message: "A megadott ID-hoz tartozó jog nem található." });
        }
