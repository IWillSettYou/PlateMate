...
export default {
  ...
  async mounted(){
    try {
        const response = await this.getInProcessOrders();
        try{
...
<template>
      <h1 v-if="!notEmpty" class="text-6xl">Nincsenek Készülő rendelések</h1>
        <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-8 max-w-fit w-full"  v-if="notEmpty">
        <h2 class="text-2xl font-semibold text-gray-700 dark:text-gray-300 mb-6 text-center">Készülő rendelések</h2>
            ...
            <tbody>
              <tr v-for="(order, index) in orders" :key="index" class="hover:bg-blue-100 border-b">
                ...
                <td class="py-4 px-4 text-sm">
                  <button @click="updateOrder(order.id)" class="bg-green-500 hover:bg-green-800 text-white font-bold py-1 px-3 rounded">
                    Kész
                  </button>
                </td>
              </tr>
